...
const NoteList = ({ notes, onNoteClick, onAddClick, loading }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {searchTerm && (
          <button
            ...
            className="absolute right-3 top-2.5 text-gray-400 hover:text-gray-600 
                       dark:hover:text-gray-200 text-sm font-medium"
          >
            Clear
          </button>
        )}
      ...
      <button
        onClick={onAddClick}
        className="fixed bottom-8 right-8 w-14 h-14 bg-blue-500 hover:bg-blue-600 
                   text-white rounded-full shadow-lg flex items-center justify-center
                   transition-all duration-300 hover:scale-110 hover:shadow-xl z-30"
      >
        <FaPlus size={24} />
      </button>
    </div>
